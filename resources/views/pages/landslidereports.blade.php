...
<div class="row">
	<div class="col-xs-12">
		<h1 class="page-header">Generate Landslide List</h1>
	</div>
		...
		<div class="col-xs-12">
			...
			<div class="col-xs-12 sorting-reports col-xs-12 well per-inputhydrometfilter">
				<div class="col-xs-12">
					<form action="{{ action('LandslideController@filterLandslideReport') }}" method="post">
			...
			<table class="table table-hover table-striped tblcontents tbl-reportgen"  id="landslidereporttable">
				<thead>
					<th>Location</th>
					<th>Province</th>
				...
				<tbody>
					@foreach($landslides as $landslide)
						<tr>
							...
							<td>
								{{$landslide->pastrainvalue}} mm
							</td>
							<td>
								@foreach($users as $user)
									@if($user->id == $landslide->user_id)

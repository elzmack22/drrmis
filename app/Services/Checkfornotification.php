...
class Checkfornotification
{
	...
	function __construct(Getcsvdataapi $AppServiceCsvfunction){
                ...
                $this->todaypath = 'drrmis.info/public/data'.'/'.date('Y').'/'.date('m').'/'.date('d').'/';
		$this->yesterday = 'drrmis.info/public/data'.'/'.date('Y/m/d',strtotime("-1 days")).'/';
		$this->lasttwodayspath = 'drrmis.info/public/data'.'/'.date('Y/m/d',strtotime("-2 days")).'/';
	...
	public function docheck(){
		...
		foreach ($sensors as $sensor) {
			...
			$firstwarn = 0;
			$firstwarn = $this->AppServiceCsvfunction->getaccum($this->todaypath,$sensor->assoc_file);	
			
			if(($firstwarn >= $threshtotal) && ($firstwarn != 0)){				
				$this->newSensorNotification($sensor->id,$firstwarn);	
			}	

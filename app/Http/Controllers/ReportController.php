...
{	
    ...
    public function getSensorThreshold(Request $request){
    	...
    	foreach ($thresholds as $threshold) {
    		if ($threshold->address_id == $sensorID){
    			...
    			$thresholdData = array($thFlood, $thLandslide, $categoryID);
    		}
    	}
			...
			case '1':
				if ($generateAllSensors == "false") {
					...
					foreach ($provinces as $province) {
						foreach ($sensors as $sensor) {
							if (($sensor->category_id == "1" || $sensor->category_id == "3") && 
										($province->id == $sensor->province_id)){
								$sensorID[] = $sensor->id;
								$categoryID[] = $sensor->category_id;
							}
						}
		    	...
		    	$data = $this->proccessData($displayType, $generateAllSensors ,$sensorType, $provinceID, $sensorID, 
    ...
    private function getDatePerDay($displayType, $dateStart, $dateEnd){
    	...
    	if ($displayType == "1" || $displayType == "2" || $displayType == "3" || $displayType == "6"){
			...
			$date_end = $dateEndArray[0] . "/" .  sprintf("%02d", $dateEndArray[1]) . "/" . 
						sprintf("%02d", $daysEndMonth);
    ...
    private function processDataRanges($displayType, $generateAllSensors, $dateStart, $dateEnd){
        ...
        if ($displayType == "1" || $displayType == "2" || $displayType == "3" || 
        	$displayType == "4" || $displayType == "5" || $displayType == "6" ||
        	$displayType == "0"){
    ...
    private function processDBData_Default_Hourly_Others($displayType, $categoryID, $sensorID, 
    													 $dateArray, $startDayDate, $endDayDate){
								...
								case '1':
									...
									if ($displayType == "1") {
										if (!$toggleNextDay && $timeHour >= 8){
								        	...
								        	$sensorData[] = array("date_read" => $tempDate,
								...
								case '4':
									...
									if ($displayType == "1"){
										...
										if (!$toggleNextDay && $timeHour >= 8){
								        	...
								        	$sensorData[] = array("date_read" => $tempDateTime,
								        					 	  "rain_value" => $rainValue,
								        					 	  "rain_intensity" => $rainIntensity,
								        					 	  "rain_duration" => $rainDuration,
								        					 	  "air_temperature" => $airTemperature,
		...
		if ($displayType == "6"){
			...
			switch ($categoryID) {
				...
				case '1':
					...
					$sensorData[] = array("date_start" => $startingDate,
								          "date_end" => $endingDate,
								          "rain_cumulative" => $cumulativeRain);
		...
		$_data = array("sensor_data" => $sensorData,
    ...
    private function processDBData_Daily($categoryID, $sensorID, $generateAllSensors, 
    									 $_dateArray, $startDayDate, $endDayDate){
    	...
    	foreach ($sensorID as $sensorkey => $_sensorID) {
	    	...
	    	$referenceHour = 8;
			$referenceHour2 = 0;
			$sensorDefectiveDate = $this->getSensorDefectiveDate($_sensorID, $startDayDate, 
														   		 $endDayDate);
	    		...
	    		switch ($categoryID[$sensorkey]) {
		    		...
		    		case '1':
		    			...
		    			if ($generateAllSensors == "false"){
		    				...
		    				$sensorData[] = array("date_read" => $dateArray[$keyDate][0],
							        		  	  "rain_cumulative" => $cumulativeRain);
    ...
    private function processDBData_Monthly($categoryID, $sensorID, $generateAllSensors, $dateStart, 
    									   $dateEnd, $dateArray, $startDayDate, $endDayDate){
					...
					switch ($categoryID[$sensorkey]) {
			    		...
			    		case '1':
			    			...
			    			if ($generateAllSensors == "false"){
			    				...
			    				$sensorData[] = array("date_read" => $monthDate,
								        		  	  "rain_cumulative" => $cumulativeRain);
    ...
    private function processDBData_Yearly($categoryID, $sensorID, $generateAllSensors, $dateStart, 
    									  $dateEnd, $dateArray, $startDayDate, $endDayDate){
				...
				switch ($categoryID[$sensorkey]) {
			    	...
			    	case '1':
			    		...
			    		if ($generateAllSensors == "false"){
			    			...
			    			$sensorData[] = array("date_read" => $yearDate,
								        		  "rain_cumulative" => $cumulativeRain);
    ...
    private function getDBData($displayType, $generateAllSensors, $categoryID, $sensorID, $dateArray, 
    						   $dateStart, $dateEnd){
		...
		switch ($displayType) {
			...
			case '1':
				...
				$data = $this->processDBData_Default_Hourly_Others($displayType, $categoryID[0], $sensorID[0], 
																   $dateArray, $startDayDate, $endDayDate);
			...
			case '3':
				...
				$data = $this->processDBData_Daily($categoryID, $sensorID, $generateAllSensors,
												   $dateArray, $startDayDate, $endDayDate);
			...
			case '4':
				...
				$data = $this->processDBData_Monthly($categoryID, $sensorID, $generateAllSensors, $dateStart, 
													 $dateEnd, $dateArray, $startDayDate, $endDayDate);
			...
			case '5':
				...
				$data = $this->processDBData_Yearly($categoryID, $sensorID, $generateAllSensors, 
													$dateStart, $dateEnd, $dateArray, $startDayDate, $endDayDate);
    ...
    private function proccessData($displayType, $generateAllSensors, $sensorType, $provinceID, $sensorID, 
    							  $categoryID, $dateStart, $dateEnd){
        ...
        $data = $this->getDBData($displayType, $generateAllSensors, $categoryID, $sensorID, $dateArray, 
        						 $dateStart, $dateEnd);
